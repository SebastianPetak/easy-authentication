...
module.exports = function(passport) {
	...
	passport.use('local-signup', new LocalStrategy({
		// by default, local strategy uses username nad password, we will
		// override with email
		usernameField : 'email',
	...
	function(req, email, password, done) {
		...
		// asynchronous
		// User.fineOne won't fire unless data is sent back
		process.nextTick(function() {

			// find a user whose email is the same as the forms email
			// we are checking to see if the user trying to login already exists
			User.findOne({ 'local.email' : email }, function(err, user) {
				// if there are any erros return the error
				if (err) {
					return done(err);
				}
	...
	function(req, email, password, done) {
		...
		User.findOne({ 'local.email' : email }, function(err, user) {
			...
			if (!user.validPassword(password)) {
				...
				// create the loginMessage and save it tot he session as flashdata
			}
